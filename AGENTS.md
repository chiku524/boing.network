...
- Tutorial-style pool and native DEX bootstrap flows live under `examples/native-boing-tutorial/scripts/`; operators run these against a live RPC—`deploy-native-dex-full-stack.mjs` can optionally kickstart liquidity (for example vault `deposit_add` or pool `add_liquidity` via env). The `boing-node` binary does not automatically create pools or seed reserves on first start.
