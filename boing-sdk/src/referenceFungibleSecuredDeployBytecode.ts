...
function keyLastByte(b: number): Uint8Array {
  const k = new Uint8Array(32);
  k[31] = b & 0xff;
  return k;
}

function wordZero(): Uint8Array {
  return new Uint8Array(32);
}

function wordOne(): Uint8Array {
  const w = new Uint8Array(32);
  w[31] = 1;
  return w;
}

function wordU32(n: number): Uint8Array {
  const w = new Uint8Array(32);
  new DataView(w.buffer).setUint32(28, n >>> 0, false);
  return w;
}

function wordU64(n: bigint): Uint8Array {
  const w = new Uint8Array(32);
...
function push32(w: Uint8Array): Uint8Array {
...
function emitRuntimeToMemory(runtime: Uint8Array): Uint8Array {
 ...
 */
export function referenceFungibleSecuredInitBytecode(
  config: ReferenceFungibleSecuredConfigBytes,
  runtime: Uint8Array,
): Uint8Array {
...
export function referenceFungibleSecuredDeployBytecode(
  config: ReferenceFungibleSecuredConfigBytes,
  runtime?: Uint8Array,
): Uint8Array {
  const rt =
...
export function referenceFungibleSecuredDeployBytecodeHex(
  config: ReferenceFungibleSecuredConfigBytes,
  runtime?: Uint8Array,
): `0x${string}` {
  return bytesToHex(referenceFungibleSecuredDeployBytecode(config, runtime)) as `0x${string}`;
}
...
function parseDecimalU128(label: string, s: string): bigint {
  const t = s.trim();
  if (!t) return 0n;
  if (!/^[0-9]+$/.test(t)) {
...
function parseDecimalU64(label: string, s: string): bigint {
  const t = s.trim();
  if (!t) return 0n;
  if (!/^[0-9]+$/.test(t)) {
    throw new Error(`${label}: expected non-negative decimal integer, got ${JSON.stringify(s)}`);
 ...
 */
export function referenceFungibleSecuredConfigFromNativeTokenSecurity(
  input: NativeTokenSecurityFeaturesInput | undefined,
  ctx: SecuredWizardBuildContext = {},
): ReferenceFungibleSecuredConfigBytes {
  ...
  if (wantWalletCap) {
    ...
    if (pctRaw && supply !== undefined && supply > 0n) {
      ...
      throw new Error(
        'maxWallet / antiWhale: set maxWalletPercentage and mintFirstTotalSupplyWei on the deploy builder, or set maxTxAmount to use as the per-wallet cap',
      );
    }
  }
  ...
  if (n.timelock) {
    ...
    if (ctx.chainHeight === undefined) {
      throw new Error(
        'nativeTokenSecurity.timelock: pass chainContext.chainHeight (from boing_chainHeight) so transfer unlock height can be set on-chain',
      );
    }
...
export function buildReferenceFungibleSecuredDeployBytecodeHexFromNativeTokenSecurity(
  input: NativeTokenSecurityFeaturesInput | undefined,
  ctx: SecuredWizardBuildContext = {},
): `0x${string}` {
