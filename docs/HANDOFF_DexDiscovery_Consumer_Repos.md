...
**SDK:** **`boing-sdk@^0.3.0`** on [npm](https://www.npmjs.com/package/boing-sdk) — `BoingClient.listDexPoolsPage`, `listDexTokensPage`, `getDexToken`, related types (`DexPoolListRow`, `DexTokenListRow`, diagnostics types).
...
2. **Dependency** — Pin **`boing-sdk@^0.3.0`** (or newer). Breaking note for TS: **`DexPoolListRow`** requires **`tokenADecimals`** / **`tokenBDecimals`** if you construct rows locally; parsing node JSON is unchanged for current nodes.
...
| Area | Action |
|------|--------|
| **Dependencies** | Bump **`boing-sdk`** to **`^0.3.0`**; run through swap / pool flows. |
