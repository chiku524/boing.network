...
- **Observability:** `GET /v1/status` — cursor vs chain tip, pending block estimate, effective ingest batch sizes, **`lastSync`** telemetry from the last successful (or failed) ingest.
- **Liveness:** `GET /health?deep=1` — confirms D1 is reachable from the Worker.
- **Single contract:** `GET /v1/contract/0x…` (64 hex chars) when you only need one predicted deploy row.
- **Filters:** `GET /v1/deployments/by-tx/…`, **`by-block/{height}`**, **`by-sender/…`** (indexed; see Worker README).
